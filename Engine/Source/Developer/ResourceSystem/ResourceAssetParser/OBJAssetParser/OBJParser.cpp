	...
	namespace ResourceAssetParser
	{
			...
			while (index != size)
			{
				...
				else if (type == "vt")
				{
					if (stage == 0)
					{
						...
						vertexIndex = 0;
						SubMeshInfo.Layout |= VertexLayouts::TexCoord;
					}
				...
				else if (type == "vn")
				{
					if (stage == 1)
					{
						...
						vertexIndex = 0;
						SubMeshInfo.Layout |= VertexLayouts::Normal;
					}
		...
		void OBJParser::Dump(ByteBuffer& Buffer, const MeshInfo& MeshInfo)
		{
			for (const auto& submesPtr : MeshInfo.SubMeshes)
			{
				...
				if (BitwiseUtils::IsEnabled(subMesh.Layout, VertexLayouts::Position))
				...
				if (BitwiseUtils::IsEnabled(subMesh.Layout, VertexLayouts::Normal))
					for (const auto& vertex : subMesh.Vertices)
					{
						Buffer.Append('v');
						Buffer.Append('n');
				...
				if (BitwiseUtils::IsEnabled(subMesh.Layout, VertexLayouts::TexCoord))
					for (const auto& vertex : subMesh.Vertices)
					{
						Buffer.Append('v');
						Buffer.Append('t');
