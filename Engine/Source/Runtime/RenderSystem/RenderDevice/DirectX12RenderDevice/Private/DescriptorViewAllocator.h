...
#define IMPLETEMENT_ALLOCATE() \
				CoreDebugAssert(Categories::RenderSystem, Handle != nullptr, "Handle cannot be null"); \
			...
			public:
				...
				{
					CoreDebugAssert(Categories::RenderSystem, !m_IsInitialized, "DescriptorViewAllocator is already initialized");
				...
				INLINE bool Deinitialize(void)
				{
					for (uint8 i = 0; i < m_DescriptInfoCount; ++i)
					{
						...
						RenderSystemAllocators::RenderSystemAllocator_Deallocate(info.AllocatedStatus);
					}
				...
				{
					CoreDebugAssert(Categories::RenderSystem, Resource != nullptr, "Resource cannot be null");
...
#undef ALLOCATE_INTERNAL
			...
			private:
				...
				{
					CoreDebugAssert(Categories::RenderSystem, m_DescriptInfoCount < RESERVED_DESCRIPTOR_HEAP_COUNT, "Reached max of reserved descriptor heaps");
