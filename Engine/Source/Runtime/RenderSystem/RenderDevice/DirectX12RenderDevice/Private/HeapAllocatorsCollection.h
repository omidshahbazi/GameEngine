...
#include <DirectX12RenderDevice\Private\HeapAllocators.h>
		...
		namespace Private
		{
			...
			public:
				...
				INLINE bool Allocate(uint64 Size, D3D12_RESOURCE_STATES State, bool IsCPUAccessible, HeapAllocator::ResourceHandle* Handle)
				{
					...
					CoreDebugAssert(Categories::RenderSystem, allocators != nullptr, "Couldn't find best fit HeapAllocators");
				...
				INLINE bool Deallocate(const HeapAllocator::ResourceHandle& Handle)
				{
					...
					CoreDebugAssert(Categories::RenderSystem, false, "This resource doesn't allocated by this HeapAllocatorsCollection");
