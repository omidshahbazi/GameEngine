...
namespace Engine
{
	...
	namespace Platform
	{
		...
		public:
			...
			template<typename T> static T *Allocate(uint64 Length)
			{
				return reinterpret_cast<T*>(Allocate(Length * sizeof(T)));
			}

			template<typename T> static void Free(T *Address)
			{
				Free(reinterpret_cast<byte*>(Address));
			}

			template<typename T> static void Set(T *Address, int32 Value, uint64 Length)
			{
				Set(reinterpret_cast<byte*>(Address), Value, Length * sizeof(T));
			}

			template<typename T> static void Copy(const T *Source, T *Destination, uint64 Length)
			{
				Copy(Source, 0, Destination, 0, Length);
			}

			template<typename T> static void Copy(const T *Source, uint64 SourceIndex, T *Destination, uint64 DestinationIndex, uint64 Length)
			{
			...
			template<typename T> static bool AreEqual(const T *Left, const byte *T, uint64 Length)
			{
				return AreEqual(Left, 0, Right, 0, Length);
			}

			template<typename T> static bool AreEqual(const T *Left, uint64 LeftIndex, const T *Right, uint64 RightIndex, uint64 Length)
			{
