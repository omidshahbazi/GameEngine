...
namespace Engine
{
	...
	namespace ReflectionTool
	{
		...
		void ReflectionGenerator::GenerateConstructorsDefinition(String& Content, const TypeList& Types, AccessSpecifiers Access)
		{
			...
			//HITODO: it has undefiend behavior in cases with multiple constructors
			for each (auto & t in Types)
			{
				MetaConstructor* type = (MetaConstructor*)t;

				Content += "\nReturnValue=new " + topNestName + "(" + GetArgumentsDataTypeText(type->GetParameters()) + ");"; //HITODO: Must allocate in an allocator except reflection allocator
