	...
	namespace ResourceSystem
	{
		namespace Private
		{
			...
			const String BuiltInAssets::SPRITE_RENDERER_SHADER_SOURCE =
				...
				"float4 texBordersUV = texBorders / texDim4;"
				"float4 elemBorderUV = texBorders / float4(elemDim.x, elemDim.y, elemDim.x, elemDim.y);"
				"if (drawMode == 1)"
				"finalUV = float2(ProcessCoord(uv.x, elemBorderUV.x, elemBorderUV.z, texBordersUV.x, texBordersUV.z, 1), ProcessCoord(uv.y, elemBorderUV.y, elemBorderUV.w, texBordersUV.y, texBordersUV.w, 1));"
				"else if (drawMode == 2)"
				"{"
				"float2 bordersSum = float2(texBorders.x + texBorders.z, texBorders.y + texBorders.w);"
				"float2 tileCount = (elemDim - bordersSum) / (texDim - bordersSum);"
				"finalUV = float2(ProcessCoord(uv.x, elemBorderUV.x, elemBorderUV.z, texBordersUV.x, texBordersUV.z, tileCount.x), ProcessCoord(uv.y, elemBorderUV.y, elemBorderUV.w, texBordersUV.y, texBordersUV.w, tileCount.y));"
				"}"
				"}"
			...
			const String BuiltInAssets::CUBE_MESH =
			...
			const String BuiltInAssets::SPHERE_MESH =
			...
			const String BuiltInAssets::CONE_MESH =
