		...
		static int Main(string[] Args)
		{
			...
			if (IsEnumDefine<BuildSystem.TargetsToBuild>(Args[0].Substring(1)))
			{
				...
				else if (Args.Length >= 3 && IsEnumDefine<BuildSystem.PlatformArchitectures>(Args[1].Substring(1)))
				{
					...
					if (IsEnumDefine<ProjectFileGenerator.BuildConfigurations>(Args[2].Substring(1)))
					{
						ProjectFileGenerator.BuildConfigurations buildConfiguration = GetEnum<ProjectFileGenerator.BuildConfigurations>(Args[2].Substring(1));
